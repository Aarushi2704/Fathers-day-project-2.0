...
function draw() {
 ...
 if(gameState == 1){
    form.hide();
    image(bb,displayWidth/(-100),displayHeight/(-100),displayWidth/10,displayHeight/10);
...
function Question(){
  rand1 = Math.round(random(x,y));
  rand2 = Math.round(random(x,y));
  var temp;

  var questOpt = Math.round(random(1,6));
...
function EndGame(){
  imageMode(CENTER);
  image(lostImg,displayWidth/39,displayHeight/38,displayWidth/18.5,displayHeight/18.5);
  noLoop();
  gameState = 2;
}